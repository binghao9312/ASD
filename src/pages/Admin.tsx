...
import { Database, Users, Shield, ListChecks } from 'lucide-react';
...
export function Admin() {
  ...
  const [activeTab, setActiveTab] = useState<'data' | 'users' | 'roles' | 'form'>('data');
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-4 bg-white dark:bg-slate-900 rounded-xl p-1 shadow-sm border border-slate-100 dark:border-slate-800">
        <button
      ...
      <div className="pb-10">
        {activeTab === 'data' && <AdminLuggages />}
        {activeTab === 'users' && <AdminUsers />}
        {activeTab === 'roles' && canManageSettings && <AdminRoles />}
        {activeTab === 'form' && canManageSettings && <AdminForm />}
      </div>
    </div>
  );
}
