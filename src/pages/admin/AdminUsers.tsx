...
import { Users, Check, X, Shield, MapPin } from 'lucide-react';
    ...
    <div className="space-y-4 relative">
      ...
      {pendingUsers.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-sm font-semibold text-amber-700 dark:text-amber-300">待審核列表</h3>
          {pendingUsers.map((u) => (
            <div key={u.email} className="glass-card p-4 space-y-4 border border-amber-200 dark:border-amber-900/50">
              <div className="flex justify-between items-start">
                <div>
                  <div className="font-bold text-slate-800 dark:text-slate-100 text-lg">{u.name || '未設定名稱'}</div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {(u.building || u.floor) && (
                      <span className="text-xs px-2 py-1 rounded-full font-medium bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 flex items-center gap-1">
                        <MapPin className="w-3 h-3" /> {u.building} {u.floor}樓
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-3 border-t border-slate-100 dark:border-slate-800 pt-3">
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleUpdateUserStatus(u.email, 'pending')} className="flex-1 flex justify-center items-center gap-1 py-2 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-100 transition-colors text-sm font-medium">
      ...
      {approvedUsers.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-sm font-semibold text-slate-600 dark:text-slate-300">已通過列表</h3>
          {approvedUsers.map((u) => {
            const userRoleGroup = roles.find((role) => role.id === u.roleId);
            const isSuper = u.email === 'a0938676069@gmail.com' || u.roleId === 'superadmin';
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <div className="font-bold text-slate-800 dark:text-slate-100 text-lg flex items-center gap-2">
                      {u.name || '未設定名稱'}
                      {isSuper && <Shield className="w-4 h-4 text-amber-500" />}
                    </div>
                    <div className="text-sm text-slate-500 mb-2">{u.email}</div>
                    <div className="flex flex-wrap gap-2">
                      <span className="text-xs px-2 py-1 rounded-full font-medium bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300">
                        {userRoleGroup ? userRoleGroup.name : '一般使用者'}
                      </span>
                      {(u.building || u.floor) && (
                        <span className="text-xs px-2 py-1 rounded-full font-medium bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 flex items-center gap-1">
                          <MapPin className="w-3 h-3" /> {u.building} {u.floor}樓
                ...
                {!isSuper && (
                  <div className="flex flex-col gap-3 border-t border-slate-100 dark:border-slate-800 pt-3">
                    <div className="flex flex-col gap-1">
                      <label className="text-xs text-slate-500">分配身分組</label>
                      <select
                        value={u.roleId || ''}
                    ...
                    <button onClick={() => handleUpdateUserStatus(u.email, 'pending')} className="flex justify-center items-center gap-1 py-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors text-xs">
                      取消權限
                    </button>
                  </div>
                )}
      ...
      {approvingEmail && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-sm space-y-4 shadow-xl">
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">通過審核</h3>
            <p className="text-sm text-slate-500">請為 {users.find(u => u.email === approvingEmail)?.name} 選擇初始身分組</p>
            <select
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button onClick={confirmApprove} className="btn-primary flex-1">確認通過</button>
            </div>
          </div>
        </div>
      )}
